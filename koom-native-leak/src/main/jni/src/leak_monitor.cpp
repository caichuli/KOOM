...
std::vector<std::shared_ptr<AllocRecord>> LeakMonitor::GetLeakAllocs() {
  ...
  auto is_leak = [&](decltype(unreachable_allocs)::value_type &unreachable,
    ...
    //to fit address prefix 0b0000
    auto live_start = (CONFUSE(live->address) & 0xFFFFFFFFFF);
    ...
    ALOGV("unreachable first:%lx ,unreachable second:%lx", unreachable.first,unreachable.second);
    ALOGV("live_start:%lx ,live_end:%lx, unreachable_start:%lx, unreachable_end:%lx",live_start,live_end,unreachable_start,unreachable_end);
    // TODO why
    ALOGV("live_start == unreachable_start：%d, second condition：%d ",(live_start == unreachable_start),(live_start >= unreachable_start && live_end <= unreachable_end));
